...
public class CustomizeStation : MonoBehaviour
{
    [Header("UI References")]
    [Tooltip("ลาก CanvasGroup ของหน้าต่าง Customize UI มาใส่")]
    [SerializeField] private CanvasGroup customizeCanvasGroup; // (เปลี่ยนจาก minimap)
    ...
    private void Start()
    {
        ...
        if (customizeCanvasGroup == null) // (เปลี่ยน)
        {
            Debug.LogError("ยังไม่ได้ลาก Customize Canvas Group มาใส่ใน Inspector!"); // (เปลี่ยน)
        }
        else
        {
            // ซิงค์สถานะเริ่มต้น
            isOpen = customizeCanvasGroup.alpha > 0; // (เปลี่ยน)
        }
    }
    ...
    private void OpenCustomize() // (เปลี่ยน)
    {
        isOpen = true;
        customizeCanvasGroup.alpha = 1;
        customizeCanvasGroup.interactable = true;
        customizeCanvasGroup.blocksRaycasts = true;
    ...
    private void CloseCustomize() // (เปลี่ยน)
    {
        isOpen = false;
        customizeCanvasGroup.alpha = 0;
        customizeCanvasGroup.interactable = false;
        customizeCanvasGroup.blocksRaycasts = false;
